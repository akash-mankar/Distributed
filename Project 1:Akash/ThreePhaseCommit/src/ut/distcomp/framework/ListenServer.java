...
public class ListenServer extends Thread {
	...
	final int port;
	final int procNum;
	final List<ut.distcomp.framework.IncomingSock> socketList;
	final ut.distcomp.framework.Config conf;
	final ServerSocket serverSock;

	protected ListenServer(ut.distcomp.framework.Config conf, List<ut.distcomp.framework.IncomingSock> sockets) {
		this.conf = conf;
	...
	public void run() {
		while (!killSig) {
			try {
				ut.distcomp.framework.IncomingSock incomingSock = new IncomingSock(
						serverSock.accept());
